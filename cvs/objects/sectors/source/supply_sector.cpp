 ...
 * \todo Move entire calculation here once demand sectors are rewritten.
 */
double SupplySector::getPriceWithSubsidyOrTax( const GDP* aGDP, const int aPeriod ) const {
    return Sector::getPriceWithSubsidyOrTax( aGDP, aPeriod );
}
...
void SupplySector::calcFinalSupplyPrice( const GDP* aGDP, const int aPeriod ){
    ...
    double subsidizedPrice = getPriceWithSubsidyOrTax( aGDP, aPeriod);
